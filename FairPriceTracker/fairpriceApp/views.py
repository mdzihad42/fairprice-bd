...
def admin_dashboard(request):
    ...
    total_agents = User.objects.filter(role='agent', is_approved=True).count()
    total_warehouses = WarehouseModel.objects.filter(is_approved=True).count()
...
def public_dashboard(request):
    ...
    if search_query:
        ...
        crop_objects = list(crops)
        crop_names_lower = [c.name.lower() for c in crop_objects]

        close_matches = difflib.get_close_matches(
            search_lower, crop_names_lower, n=3, cutoff=0.7
        )

        if close_matches:
            matched_crops = [crop_objects[crop_names_lower.index(m)] for m in close_matches]
            crops = matched_crops
        else:
            crops = []
